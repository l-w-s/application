...
class ControlNode extends StatementNode
{
	...
	{
		if ($this->escape === null && implode('', $context->getEscapingContext()) !== Latte\Context::Html) {
			$this->escape = true;
		}
		...
		$fetchCode = $context->format(
			$this->name instanceof StringNode
				? '$ʟ_tmp = $this->global->uiControl->getComponent(%raw);'
				: 'if (!is_object($ʟ_tmp = %raw)) $ʟ_tmp = $this->global->uiControl->getComponent($ʟ_tmp);',
			$this->name,
		);
